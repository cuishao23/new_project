...
from starter.dao.new_mysql import add_g100_user_infos, get_new_user_infos, get_dec_user_infos, update_user_infos, get_user_info, get_idcard_info, update_move_member
    ...
    try:
        template = {
            'userInfo': {
                ...
                'logindays', '邮箱', '生日', 'app对应id', '登陆方式', '登陆id'),
                'field': (
                ...
                'birth', 'appid', 'login_type', 'login_id')
            }
        }
        with user_session_scope() as session:
...
def analyse_idcard():
    ...
    with user_session_scope() as session:
        ...
        pool = multiprocessing.Pool(multiprocessing.cpu_count())  # 创建一个线程池
        data = pool.map(get_card_info, user_list)  # 往线程池中填线程
        pool.close()  # 关闭线程池，不再接受线程
        pool.join()  # 等待线程池中线程全部执行完
